CREATE PROCEDURE [dbo].[GetMachineLocationList]
  (  
    @PageNumber INT,
    @PageSize INT,
...
BEGIN
    ...
    SET @DynamicSQL = 'SELECT LocationId, LocationTitle, LocationDetail, CreatedOn
   ...
   IF OBJECT_ID('tempdb..#SplitResultTable') IS NOT NULL
	...
	SELECT COUNT(LocationId) TotalCount, @PageNumber PageNumber, @PageSize PageSize FROM Location;
